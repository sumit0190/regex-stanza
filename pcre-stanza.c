...
// This is our Stanza function to add positions of matches (and their lengths)
void append_positions(int position, int size);
...
void pcre_search(char *pattern, char *subject, int find_all)
{
	...
	for (i = 0; i < rc; i++) {
		...
		append_positions(ovector[2*i], substring_length);
	...
	if (namecount <= 0) {
		...
		/* Now we can scan the table and, for each entry, get the number, the name,
		...
		for (i = 0; i < namecount; i++) {
			...
			append_positions(ovector[2*n], ovector[2*n+1] - ovector[2*n]);
